...
const DeckItem = ({deck, index}: {deck: YuGiOhUserDeck, index: number}) => {
  ...
  return (        
    <Pressable onPress={() => handlePress()} style={{marginTop: index >= DECK_GRID_COLUMNS ? 10 : 0}}>
        ...
        <View style={{width: '100%', height: hp(80)}} >
            {
                ...
                renderItem={
                    ({item, index}) => <DeckItem deck={item} width={width} height={height} index={index} />
                }
                />
            } 
        </View>
